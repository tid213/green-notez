...
function DailyCheck({session, growName}) {

    const [message, setMessage] = useState("Take measurements for daily check in");
    ...
    const handleChange = (event) => {
      ...
      const handleSubmit = async (event) => {
        ...
        const { error } = await supabase
            .from("grow_notes")
            .insert({user_id: session.user.id, grow_name: growName, note_type: "Daily Check", 
